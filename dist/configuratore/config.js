...
const initialSector = new URLSearchParams(window.location.search).get('settore');
if (initialSector && byId(SETTORI, initialSector)) {
  S.settore = initialSector;
  byId(SETTORI, initialSector).proposti.forEach(id=>{
    const mod=modById(id);
    if(mod && mod.stato!=='arrivo') S.extra.add(id);
  });
}
const FORMULE = {
